...
function MessagesContent() {
  ...
  const getMessageStatus = (msg: any) => {
    ...
    if (msg.readAt) {
      return <CheckCheck className="w-3.5 h-3.5 text-cyan-300" aria-label="Read" />;
    } else if (msg.deliveredAt) {
      return <CheckCheck className="w-3.5 h-3.5 text-white/60" aria-label="Delivered" />;
    } else {
      return <Check className="w-3.5 h-3.5 text-white/60" aria-label="Sent" />;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-paper bg-aurora">
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6">
        <div className="card rounded-3xl overflow-hidden h-[calc(100dvh-8.5rem)] flex animate-fade-up">
          {/* Sidebar — hidden on mobile while a chat is open */}
          <div className={`${selectedUser ? "hidden md:flex" : "flex"} w-full md:w-80 shrink-0`}>
            <MessagesSidebar
          ...
          <div
            className={`${selectedUser ? "flex" : "hidden md:flex"} flex-1 flex-col bg-zinc-50/80 min-w-0`}
          >
            {!selectedUser ? (
              <EmptyState />
            ) : (
