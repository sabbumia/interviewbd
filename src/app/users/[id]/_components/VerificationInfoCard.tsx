...
export default function VerificationInfoCard({
  ...
  const hasSocials = isConnected && (socials?.linkedin || socials?.github);
  ...
  return (
    <div className="card p-6 animate-fade-up" style={{ animationDelay: '140ms' }}>
      <h2 className="font-display text-lg font-bold text-ink mb-5 flex items-center gap-2">
        <IdCard className="w-5 h-5 text-brand-600" />
        Profile Information
      </h2>
      ...
      <div className="space-y-4">
        ...
        {/* Contact — gated behind connection */}
        <div className="flex items-start gap-3">
          <span className="w-9 h-9 rounded-xl bg-amber-50 border border-amber-100 text-amber-600 flex items-center justify-center shrink-0">
            {isConnected ? <Phone className="w-4.5 h-4.5" /> : <Lock className="w-4.5 h-4.5" />}
          </span>
          <div className="flex-1 min-w-0">
            <p className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest mb-0.5">
              Contact
            ...
            {isConnected ? (
              <p className="text-sm font-semibold text-ink">{verificationDetails.mobileNumber}</p>
            ) : (
              <>
                <p className="text-sm font-semibold text-zinc-300 tracking-widest select-none">
                  •••• •••• ••••
                </p>
                <p className="text-xs text-zinc-400 mt-0.5">Connect to view contact</p>
              </>
            )}
          </div>
        </div>

        {/* Social links — only visible when connected */}
        {hasSocials && (
